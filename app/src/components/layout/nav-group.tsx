...
function getUrlString(url: string | UrlObject | undefined): string {
  if (!url) return "";
  return typeof url === "string" ? url : url.pathname || "";
}

function checkIsActive(href: string, item: NavItem, mainNav = false) {
  const itemUrl = getUrlString(item.url);
  return (
    href === itemUrl || // /endpoint?search=param
    href.split("?")[0] === itemUrl || // /endpoint
    !!item?.items?.filter((i) => getUrlString(i.url) === href).length || // child nav
